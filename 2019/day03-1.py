...
def shorten(ls, p):
    ...
    for i in range(len(ls.coords)):
        if LineString([coords[-1], ls.coords[i]]).intersects(p):
            coords.append((p.x,p.y))
            return LineString(coords)
        else:
            coords.append(ls.coords[i])
